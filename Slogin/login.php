...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['signup'])) {
    ...
    if (!filter_var($Email, FILTER_VALIDATE_EMAIL)) {
        ...
        $checkQuery = "SELECT * FROM student_account WHERE Email = '$Email'";
        $checkResult = mysqli_query($con, $checkQuery);

        if (mysqli_num_rows($checkResult) > 0) {
            echo "<script>alert('Email already exists');</script>";
            echo "<script>window.location.href = 'Sprofile/Home/home.php';</script>";
        } else {
            // Insert new user into the database
            $query = "INSERT INTO student_account (Email, Password) VALUES ('$Email', '$Password')";
            if (mysqli_query($con, $query)) {
              echo "<script>alert('Successfully Created Account');</script>";
              echo "<script>window.location.href = 'Sprofile/profile.php';</script>";
          } else {
...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['login'])) {
    ...
    // Assume $userID contains the user's ID obtained during login
    ...
    if (empty($RegNo) || empty($Password)) {
        ...
        $profileQuery = "SELECT * FROM student_profile WHERE RegNo = '$RegNo'";
        ...
        if (mysqli_num_rows($profileResult) == 1) {
            ...
            $signupQuery = "SELECT sp.*, sa.Password 
                            ...
                            JOIN student_account sa ON sp.email = sa.Email
...
<main>
    <div class="box">
        <div class="inner-box">
            ...
            <div class="carousel">
                <img src="login.avif" alt="student">
            </div>
        </div>
    </div>
</main>
